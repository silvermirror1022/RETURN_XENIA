...
protected:
	...
	TSubclassOf<class UUserWidget> RXGameMainMenuWidgetClass; // 플레이어에게 보일 게임메인메뉴 UI 위젯 BP

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "MainMenu")
	TObjectPtr<class UUserWidget> ARXGameMainMenuWidget; // HUD UI 포인터

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "PauseMenu")
	TSubclassOf<class UUserWidget> RXGamePauseMenuWidgetClass; // 플레이어에게 보일 게임팝업메뉴 UI 위젯 BP

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "PauseMenu")
	TObjectPtr<class UUserWidget> ARXGamePauseMenuWidget; // HUD UI 포인터
